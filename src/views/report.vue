        ...
        <v-row>
            ...
            <v-col v-else cols=12>
                <v-card flat>
                    ...
                    item-key="id" loading="true" :search="search">
...
export default {
    name:'TicketManager',
    components:{Ticket,delConfirm},
    mounted(){
        this.fetchrecords();
        document.title = 'StockMKT Challenge - Quick Book'
    },
    data(){
        return{
            gloabl:this,
            ...
            fab:false,
            header:[
                {text:'Access code',value:'ticketcode'},
                {text:'Number',value:'ticketnumber'},
                {text:'Start At',value:'startat'},
                {text:'End At',value:'endat'},
                {text:'Player',value:'player'},
                {text:'Game',value:'player'},
    ...
    methods:{
         async statusUpdate(data,id){
            if(data == 1){
                const response =  await this.$http.post(process.env.VUE_APP_API+'/ticket/status',{
                    id:id,
                    status:0
                });
